...
class AdminHotelGeneralSettingsController extends ModuleAdminController
{
    ...
    public function beforeUpdateOptions()
    {
        if (isset($_POST['PS_SHOP_STATE_ID']) && $_POST['PS_SHOP_STATE_ID'] != '0') {
            // PostgreSQL version with PDO and prepared statements
            try {
                $pdo = new PDO('pgsql:host=localhost;dbname=qloapps;port=5432', 'username', 'password');
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                
                // Use double quotes for PostgreSQL identifiers and remove backticks
                $sql = 'SELECT "active" FROM "' . _DB_PREFIX_ . 'state" 
                        WHERE "id_country" = :id_country 
                        AND "id_state" = :id_state';
                        
                $stmt = $pdo->prepare($sql);
                $stmt->bindParam(':id_country', (int)Tools::getValue('PS_SHOP_COUNTRY_ID'), PDO::PARAM_INT);
                $stmt->bindParam(':id_state', (int)Tools::getValue('PS_SHOP_STATE_ID'), PDO::PARAM_INT);
                $stmt->execute();
                
                $isStateOk = $stmt->fetchColumn();
                
                if ($isStateOk != 1) {
                    $this->errors[] = Tools::displayError('The specified state is not located in this country.');
                }
            } catch (PDOException $e) {
                $this->errors[] = Tools::displayError('Database error: ' . $e->getMessage());
            }
        }
    }
