...
class BlockMyAccount extends Module
{
	...
	{
		return Db::getInstance()->execute('INSERT IGNORE INTO hook` (`name`, `title`, `description`, `position`) VALUES (\'displayMyAccountBlock\', \'My account block\', \'Display extra informations inside the "my account" block\', 1)');
	}

	private function removeMyAccountBlockHook()
	{
		return Db::getInstance()->execute('DELETE FROM hook` WHERE `name` = \'displayMyAccountBlock\'');
	}
