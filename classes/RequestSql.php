...
class RequestSqlCore extends ObjectModel
{
    ...
    {
        if (!$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM request_sql` ORDER BY `id_request_sql`')) {
            return false;
        }
    ...
    {
        return Db::getInstance()->executeS('SELECT `sql` FROM request_sql` WHERE `id_request_sql` = '.(int)$id);
    }
