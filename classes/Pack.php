...
class PackCore extends Product
{
    ...
    public static function isPack($id_product)
    {
        ...
        if (!array_key_exists($id_product, self::$cacheIsPack)) {
            $result = Db::getInstance()->getValue('SELECT COUNT(*) FROM pack` WHERE id_product_pack = '.(int)$id_product);
    ...
    public static function isPacked($id_product, $id_product_attribute = false)
    {
        ...
        if ($id_product_attribute === false) {
            ...
            if (!array_key_exists($cache_key, self::$cacheIsPacked)) {
                $result = Db::getInstance()->getValue('SELECT COUNT(*) FROM pack` WHERE id_product_item = '.(int)$id_product);
            ...
            if (!array_key_exists($cache_key, self::$cacheIsPacked)) {
                $result = Db::getInstance()->getValue('SELECT COUNT(*) FROM pack` WHERE id_product_item = '.((int)$id_product).' AND
    ...
    public static function getItems($id_product, $id_lang)
    {
        ...
        $result = Db::getInstance()->executeS('SELECT id_product_item, id_product_attribute_item, quantity FROM pack` where id_product_pack = '.(int)$id_product);
        $array_result = array();
        foreach ($result as $row) {
            ...
            if (isset($row['id_product_attribute_item']) && $row['id_product_attribute_item']) {
                $sql = 'SELECT agl.`name` AS group_name, al.`name` AS attribute_name
					...
					LEFT JOIN attribute_lang` al ON (a.`id_attribute` = al.`id_attribute` AND al.`id_lang` = '.(int)Context::getContext()->language->id.')
					LEFT JOIN attribute_group_lang` agl ON (ag.`id_attribute_group` = agl.`id_attribute_group` AND agl.`id_lang` = '.(int)Context::getContext()->language->id.')
    ...
    public static function getItemTable($id_product, $id_lang, $full = false)
    {
        ...
        $sql = 'SELECT p.*, product_shop.*, pl.*, image_shop.`id_image` id_image, il.`legend`, cl.`name` AS category_default, a.quantity AS pack_quantity, product_shop.`id_category_default`, a.id_product_pack, a.id_product_attribute_item
				FROM pack` a
				LEFT JOIN product` p ON p.id_product = a.id_product_item
        ...
        foreach ($result as &$line) {
            if (Combination::isFeatureActive() && isset($line['id_product_attribute_item']) && $line['id_product_attribute_item']) {
				...
				FROM product_attribute` pa
				'.Shop::addSqlAssociation('product_attribute', 'pa').'
				LEFT JOIN product_attribute_combination` pac ON pac.`id_product_attribute` = '.$line['id_product_attribute_item'].'
				LEFT JOIN attribute` a ON a.`id_attribute` = pac.`id_attribute`
				LEFT JOIN attribute_group` ag ON ag.`id_attribute_group` = a.`id_attribute_group`
				LEFT JOIN attribute_lang` al ON (a.`id_attribute` = al.`id_attribute` AND al.`id_lang` = '.(int)Context::getContext()->language->id.')
				LEFT JOIN attribute_group_lang` agl ON (ag.`id_attribute_group` = agl.`id_attribute_group` AND agl.`id_lang` = '.(int)Context::getContext()->language->id.')
				LEFT JOIN product_attribute_image` pai ON ('.$line['id_product_attribute_item'].' = pai.`id_product_attribute`)
    ...
    public static function deleteItems($id_product)
    {
        return Db::getInstance()->update('product', array('cache_is_pack' => 0), 'id_product = '.(int)$id_product) &&
            Db::getInstance()->execute('DELETE FROM pack` WHERE `id_product_pack` = '.(int)$id_product) &&
    ...
    {
        Db::getInstance()->execute('INSERT INTO pack` (`id_product_pack`, `id_product_item`, `id_product_attribute_item`, `quantity`)
		(SELECT '.(int)$id_product_new.', `id_product_item`, `id_product_attribute_item`, `quantity` FROM pack` WHERE `id_product_pack` = '.(int)$id_product_old.')');
    ...
    public static function getPacksContainingItem($id_item, $id_attribute_item, $id_lang)
    {
        ...
        $query = 'SELECT `id_product_pack`, `quantity` FROM pack`
