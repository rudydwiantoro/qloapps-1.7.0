...
class GroupReductionCore extends ObjectModel
{
    ...
    protected function _setCache()
    {
        ...
        if (count($values)) {
            $query = 'INSERT INTO product_group_reduction_cache` (`id_product`, `id_group`, `reduction`)
    ...
    public static function getGroupReductions($id_group, $id_lang)
    {
        ...
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN category_lang` cl ON (cl.`id_category` = gr.`id_category` AND cl.`id_lang` = '.(int)$lang.')
    ...
    {
        $query = 'DELETE FROM product_group_reduction_cache` WHERE `id_product` = '.(int)$id_product;
    ...
    public static function duplicateReduction($id_product_old, $id_product)
    {
        ...
        foreach ($res as $row) {
            $query .= 'INSERT INTO product_group_reduction_cache` (`id_product`, `id_group`, `reduction`) VALUES ';
    ...
    {
        $query = 'DELETE FROM group_reduction` WHERE `id_category` = '.(int)$id_category;
        if (Db::getInstance()->Execute($query) === false) {
            return false;
        }
        return true;
