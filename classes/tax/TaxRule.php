...
class TaxRuleCore extends ObjectModel
{
    ...
    public static function getTaxRulesByGroupId($id_lang, $id_group)
    {
		...
		SELECT g.`id_tax_rule`,
				 ...
				 g.`id_state`
		FROM tax_rule` g
		LEFT JOIN country_lang` c ON (g.`id_country` = c.`id_country` AND `id_lang` = '.(int)$id_lang.')
		LEFT JOIN state` s ON (g.`id_state` = s.`id_state`)
		LEFT JOIN tax` t ON (g.`id_tax` = t.`id_tax`)
		WHERE `id_tax_rules_group` = '.(int)$id_group.'
    ...
    */
    public static function isTaxInUse($id_tax)
    {
        ...
        if (!Cache::isStored($cache_id)) {
            $result = (int)Db::getInstance()->getValue('SELECT COUNT(*) FROM tax_rule` WHERE `id_tax` = '.(int)$id_tax);
