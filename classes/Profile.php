...
class ProfileCore extends ObjectModel
{
    ...
    */
    public static function getProfiles($id_lang)
    {
		...
		FROM profile` p
		LEFT JOIN profile_lang` pl ON (p.`id_profile` = pl.`id_profile` AND `id_lang` = '.(int)$id_lang.')
    ...
    */
    public static function getProfile($id_profile, $id_lang = null)
    {
        ...
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			SELECT `name`
			FROM profile` p
			LEFT JOIN profile_lang` pl ON (p.`id_profile` = pl.`id_profile`)
    ...
    public function delete()
    {
        ...
        if (empty($profileEmployees)) {
            if (parent::delete()) {
                return (
                    Db::getInstance()->execute('DELETE FROM access` WHERE `id_profile` = '.(int)$this->id)
                    && Db::getInstance()->execute('DELETE FROM module_access` WHERE `id_profile` = '.(int)$this->id)
                );
            }
        }
