...
function add_order_state($conf_name, $name, $invoice, $send_email, $color, $unremovable, $logable, $delivery, $template = null)
{
    ...
    $res &= Db::getInstance()->execute('
		INSERT INTO order_state` (`invoice`, `send_email`, `color`, `unremovable`, `logable`, `delivery`)
    ...
    foreach ($languages as $lang) {
        ...
        $res &= Db::getInstance()->execute('
		INSERT IGNORE INTO order_state_lang` (`id_lang`, `id_order_state`, `name`, `template`)
		VALUES ('.(int)$lang['id_lang'].', '.(int)$id_order_state.', "'. $name .'", "'. $template .'")
		');
    }

    $exist = Db::getInstance()->getValue('SELECT `id_configuration` FROM configuration` WHERE `name` = \''.pSQL($conf_name).'\'');
    if ($exist) {
        $res &= Db::getInstance()->execute('UPDATE configuration` SET value = "'.(int)$id_order_state.'" WHERE `name` LIKE \''.pSQL($conf_name).'\'');
    } else {
        $res &= Db::getInstance()->execute('INSERT INTO configuration` (name, value) VALUES ("'.pSQL($conf_name).'", "'.(int)$id_order_state.'"');
    }
}
