...
function setAllGroupsOnHomeCategory()
{
    $ps_lang_default = Db::getInstance()->getValue('SELECT value
		FROM configuration` WHERE name="PS_LANG_DEFAULT"');

    $results = Db::getInstance()->executeS('SELECT id_group FROM group`');
    $groups = array();
    ...
    if (is_array($groups) && count($groups)) {
        // cleanGroups
        Db::getInstance()->execute('DELETE FROM category_group`
			WHERE `id_category` = 1');
        // addGroups($groups);
