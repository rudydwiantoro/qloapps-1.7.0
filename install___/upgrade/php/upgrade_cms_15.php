...
function upgrade_cms_15()
{
    ...
    $res &= Db::getInstance()->execute('CREATE TABLE _cms_shop` (
...
`id_shop` INT( 11 ) UNSIGNED NOT NULL ,
  PRIMARY KEY (`id_cms`, `id_shop`),
    ...
    $res &= Db::getInstance()->execute('INSERT INTO _cms_shop` (id_shop, id_cms)
    ...
    $id_module_cms = Db::getInstance()->getValue('SELECT id_module from _module
		WHERE name="blockcms"`');
    if (!$id_module_cms) {
        return $res;
    }
    // /!\ : _cms_block is the wrong table name (fixed in 1.5.0.12.sql : upgrade_cms_15_rename() )
    $res &= Db::getInstance()->execute('ALTER TABLE ._cms_block`
