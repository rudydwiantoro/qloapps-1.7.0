...
function update_order_canada()
{
    ...
    if (!count($table)) {
        ...
        $sql = 'SELECT `id_order`, `price_display_method`
					FROM orders` o
					LEFT JOIN customer` cus ON (o.id_customer = cus.id_customer)
					LEFT JOIN group` g ON (g.id_group = cus.id_default_group)
					LEFT JOIN address` a ON (a.`id_address` = o.`'.$address_field.'`)
        ...
        if (!empty($values)) {
            ...
            Db::getInstance()->execute('
			INSERT INTO order_tax` (id_order, tax_name, tax_rate, amount)
			VALUES '.$values);
        }
    }
}
