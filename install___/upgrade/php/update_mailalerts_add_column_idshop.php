...
{
    $installed = Db::getInstance()->getValue('SELECT id_module FROM  module` WHERE name = "mailalerts"');
    $installed &= Db::getInstance()->getValue('SHOW TABLES LIKE "'._DB_PREFIX_.'mailalert_customer_oos"');
    if ($installed && !Db::getInstance()->execute('ALTER TABLE mailalert_customer_oos` ADD COLUMN `id_shop` int(11) NOT NULL default "0" AFTER `id_customer`')) {
