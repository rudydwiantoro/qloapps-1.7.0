...
function create_multistore()
{
	...
	WHERE id_shop = 1');
    $shop_domain = Db::getInstance()->getValue('SELECT `value` FROM configuration` WHERE `name` = "PS_SHOP_DOMAIN"');
    $shop_domain_ssl = Db::getInstance()->getValue('SELECT `value` FROM configuration` WHERE `name` = "PS_SHOP_DOMAIN_SSL"');
    ...
    $res &= Db::getInstance()->execute('
	INSERT INTO shop_url` (`id_shop`, `domain`, `domain_ssl`, `physical_uri`, `virtual_uri`, `main`, `active`)
	VALUES(1, \''.pSQL($shop_domain).'\', \''.pSQL($shop_domain_ssl).'\', \''.pSQL($physical_uri).'\', \'\', 1, 1)');

    // Stock conversion
    $sql = 'INSERT INTO stock` (`id_product`, `id_product_attribute`, `id_group_shop`, `id_shop`, `quantity`)
	VALUES (SELECT p.`id_product`, 0, 1, 1, p.`quantity` FROM product` p);';
    $res &= Db::getInstance()->execute($sql);

    $sql = 'INSERT INTO stock` (`id_product`, `id_product_attribute`, `id_group_shop`, `id_shop`, `quantity`)
	VALUES (SELECT `id_product`, `id_product_attribute`, 1, 1, `quantity` FROM product_attribute` p);';
    $res &= Db::getInstance()->execute($sql);
