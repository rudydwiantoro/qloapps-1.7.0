...
{
    $index = Db::getInstance()->executeS('SHOW INDEX FROM module` WHERE Key_name = "name"');
    if (is_array($index) && count($index)) {
        Db::getInstance()->execute('ALTER IGNORE TABLE module` DROP INDEX `name`');
    }
    ...
    Db::getInstance()->execute('ALTER IGNORE TABLE module` ADD UNIQUE `name` (`name`)');
    if (is_array($tmp) && $tmp) {
