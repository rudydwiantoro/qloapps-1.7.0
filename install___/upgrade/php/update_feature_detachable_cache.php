...
function update_feature_detachable_cache()
{
    ...
    foreach ($array_features as $config_key => $feature) {
        ...
        $exist = Db::getInstance()->getValue('SELECT `id_configuration` FROM configuration` WHERE `name` = \''.pSQL($config_key).'\'');
        if ($exist) {
            $res &= Db::getInstance()->execute('UPDATE configuration` SET value = "'.(int)$count.'" WHERE `name` = \''.pSQL($config_key).'\'');
        } else {
            $res &= Db::getInstance()->execute('INSERT INTO configuration` (name, value) values ("'.pSQL($config_key).'", "'.(int)$count.'")');
        }
    }
    return $res;
}
