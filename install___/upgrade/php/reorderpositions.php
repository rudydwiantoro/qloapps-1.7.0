...
function reorderpositions()
{
    ...
    $cat = Db::getInstance()->executeS('SELECT id_category FROM category`');
    if ($cat) {
    ...
    $cat_parent = Db::getInstance()->executeS('SELECT DISTINCT c.id_parent FROM category` c WHERE id_category != 1');
    foreach ($cat_parent as $parent) {
        $result = Db::getInstance()->executeS('
							SELECT DISTINCT c.*, cl.*
							FROM category` c
							LEFT JOIN category_lang` cl ON (c.`id_category` = cl.`id_category` AND `id_lang` = '.$ps_lang_default.')
        ...
        foreach ($result as $i => $categ) {
            Db::getInstance()->execute('UPDATE category` c
			LEFT JOIN category_lang` cl ON (c.`id_category` = cl.`id_category`)
    ...
    /* Clean CMS positions */
    $cms_cat = Db::getInstance()->executeS('SELECT id_cms_category FROM cms_category` WHERE active=1');
    if ($cms_cat) {
