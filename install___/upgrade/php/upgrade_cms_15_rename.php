...
function upgrade_cms_15_rename()
{
    ...
    if ($res) {
        $res &= $db->execute('RENAME TABLE _cms_shop` to cms_shop`');
        // in case the script upgrade_cms_15.php have set a wrong table name, it's empty
        $res &= Db::getInstance()->execute('INSERT INTO cms_shop` (id_shop, id_cms)
        ...
        $res &= Db::getInstance()->execute('ALTER TABLE cms`
