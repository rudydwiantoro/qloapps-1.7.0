...
function p15013_add_missing_columns()
{
    $errors = array();
    $db = Db::getInstance();
    $id_module = $db->getValue('SELECT id_module FROM module` WHERE name="statssearch"');

    if ($id_module) {
        $list_fields = Db::getInstance()->executeS('SHOW FIELDS FROM statssearch`');
        foreach ($list_fields as $k => $field) {
            $list_fields[$k] = $field['Field'];
        }

        if (!in_array('id_group_shop', $list_fields)) {
            if (!Db::getInstance()->execute('ALTER TABLE statssearch`
