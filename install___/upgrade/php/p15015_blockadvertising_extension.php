    ...
    if (@file_exists(_PS_ROOT_DIR_.'/modules/blockadvertising')) {
        foreach (@scandir(_PS_ROOT_DIR_.'/modules/blockadvertising') as $file) {
            if (in_array($file, array('advertising.jpg', 'advertising.gif', 'advertising.png'))) {
                $exist = Db::getInstance()->getValue('SELECT `id_configuration` FROM configuration` WHERE `name` = \'BLOCKADVERT_IMG_EXT\'');
                if ($exist) {
                    Db::getInstance()->execute('UPDATE configuration` SET value = "'.pSQL(substr($file, strrpos($file, '.') + 1)).'" WHERE `name` = \'BLOCKADVERT_IMG_EXT\'');
                } else {
                    Db::getInstance()->execute('INSERT INTO configuration` (name, value) VALUES ("BLOCKADVERT_IMG_EXT", "'.pSQL(substr($file, strrpos($file, '.') + 1)).'"');
                }
            }
        }
    }
