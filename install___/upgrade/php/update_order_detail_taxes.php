...
function update_order_detail_taxes()
{
	...
	SELECT `id_order_detail`, `tax_name`, `tax_rate` FROM order_detail`
	');
    $id_lang_list = Db::getInstance()->executeS('SELECT id_lang FROM lang`');
    ...
    foreach ($order_detail_taxes as $order_detail_tax) {
        ...
        if ($create_tax) {
            ...
            'INSERT INTO tax` (`rate`, `active`, `deleted`)
            ...
            foreach ($id_lang_list as $id_lang) {
                Db::getInstance()->execute('
				INSERT INTO tax_lang` (`id_tax`, `id_lang`, `name`)
        ...
        Db::getInstance()->execute('
		INSERT INTO order_detail_tax` (`id_order_detail`, `id_tax`)
