...
function editorial_update()
{
    ...
    if (Db::getInstance()->getValue('SELECT `id_module` FROM module` WHERE `name`="editorial"')) {
        ...
        if (file_exists(dirname(__FILE__).'/../../../modules/editorial/editorial.xml')) {
            ...
            Db::getInstance()->execute('
			INSERT INTO editorial`(`id_editorial`, `body_home_logo_link`) VALUES(1, "'.(isset($xml->body->home_logo_link) ? pSQL($xml->body->home_logo_link) : '').'")');

            $languages = Db::getInstance()->executeS('SELECT * FROM lang`');
            foreach ($languages as $language) {
                Db::getInstance()->execute('
				INSERT INTO editorial_lang` (`id_editorial`, `id_lang`, `body_title`, `body_subheading`, `body_paragraph`, `body_logo_subheading`)
